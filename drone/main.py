...
if __name__ == '__main__':
    try:
        while True:
            # Leer datos de ubicación del GPS
            latitud, longitud, altitud = leer_gps()
            print(f"Latitud: {latitud}, Longitud: {longitud}, Altitud: {altitud}")

            # Leer datos de aceleración y giro del MPU-6050
            aceleracion_x, aceleracion_y, aceleracion_z = leer_mpu6050()
            print(f"Aceleración X: {aceleracion_x}, Aceleración Y: {aceleracion_y}, Aceleración Z: {aceleracion_z}")

            # Leer datos de presión y altitud del BMP180
            presion, altitud_bmp = leer_bmp180()
            print(f"Presión: {presion}, Altitud: {altitud_bmp}")

            # Controlar motores (ejemplo: hacer girar el dron)
            controlar_motores(True, False, True, False)
